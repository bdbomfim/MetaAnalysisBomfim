##Nutrient flux and concentration Resilience data####
...
str(nutrec)#2430
unique(levels(as.factor(nutrec$Case_study)))
...
unique(levels(as.factor(res_pf$Case_study)))
...
str(data_esall_amb_Pflux)#316 obs. including all litterfall mass fractions
data_esall_amb_Pflux$Fraction
...
levels(Obs_res_Pflux$Months)

##Calculating Pantropical P flux for Total
...
str(data_es_totPflux_1)#8
unique(levels(as.factor(data_es_totPflux_1$Case_study)))#no need for random effects

totPflux_meta_1<- rma.mv(yi,vi,#random = ~(1|Site),
                    tdist = TRUE,
...
str(data_es_totPflux_8)
unique(levels(as.factor(data_es_totPflux_8$Case_study)))
...
Fig_res_amb_Pflux<-Fig_res_amb_Pflux+geom_point(aes(group=Case_study,col=Country,size=Se,shape=Fraction),stroke=1.3,alpha=0.5)+scale_color_manual(values=c("#1dabe6","#ffa600","black","#665191","#af060f"))##167923
...
Fig_res_amb_Pflux<-Fig_res_amb_Pflux+theme_bw()+geom_segment(aes(x=1, y=0, xend=36, yend=0), lty=2, color = "magenta", cex=1.4)+
  ...
  xlab("Time since disturbance (Months)")+
...
unique(levels(as.factor(data_es_totNflux_1$Case_study)))
...
str(data_es_totNflux_8)
unique(levels(as.factor(data_es_totNflux_8$Case_study)))
...
Fig_res_amb_Nflux<-Fig_res_amb_Nflux+geom_point(aes(group=Case_study,col=Country,size=Se,shape=Fraction),stroke=1.3,alpha=0.5)+scale_color_manual(values=c("#1dabe6","#ffa600","black","#665191","#af060f"))##1C39A8
...
Fig_res_amb_Nflux<-Fig_res_amb_Nflux+theme_bw()+scale_shape_discrete(solid=F)+geom_segment(aes(x=1, y=0, xend=36, yend=0), lty=2, color = "magenta", cex=1.4)+
  scale_x_discrete(breaks = c(1, 3, 6, 9,12, 15, 18,21,24,27,30,33,36))+
  ylim(-3.5,3)+ scale_fill_discrete(breaks=c("Total","Leaf","Wood","FFS"))+
  ylab(expression(Resilience~(ln~litterfall[tx]/litterfall[t0])))+#ylab(expression(Resilience~(ln~Litterfall~t[x]~t[0]^-1)))+
  xlab("Time since disturbance (Months)")+
  theme(axis.title.x =element_text(vjust = 0.5,size=24),
        axis.text.x =element_text(vjust = 1,size=22),
...
Fig_res_amb_Nflux<-Fig_res_amb_Nflux+
  ...
  guides(shape = guide_legend(nrow = 4, byrow = T, override.aes=list(size=4)),size=FALSE) +
  theme(legend.direction = "vertical", legend.box = "vertical")+labs(color="",shape="")
Fig_res_amb_Nflux<-Fig_res_amb_Nflux+xlab("Time since disturbance (Months)")
Fig_res_amb_Nflux
...
Fig_res_amb_Pc<-Fig_res_amb_Pc+geom_point(aes(group=Case_study,col=Country,size=Se,shape=Fraction),stroke=1.3,alpha=0.5)+scale_color_manual(values=c("#ffa600","black","#665191","#af060f"))##167923 "#1dabe6","#b35a2d","#c3ced0","#ffa600","#665191","#af060f"        #"#ffa600","black","#665191","#af060f"
...
Fig_res_amb_Pc<-Fig_res_amb_Pc+theme_bw()+scale_shape_discrete(solid=F)+geom_segment(aes(x=1, y=0, xend=36, yend=0), lty=2, color = "magenta", cex=1.4)+
  ...
  ylab(expression(Resilience~(ln~litterfall[tx]/litterfall[t0])))+#ylab(expression(Resilience~(ln~Litterfall~t[x]~t[0]^-1)))+xlab("")+
  theme(axis.title.x =element_text(vjust = 0.5,size=24),
        axis.text.x =element_text(vjust = 1,size=22),
        axis.title.y =element_text(vjust = 1,size=24),strip.background = element_rect(color="white", fill="white",linetype="solid"),
        axis.text.y=element_text(size=22),legend.text =  element_text(size=26),
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+guides(color = guide_legend(override.aes = list(size = 3)),shape = guide_legend(override.aes = list(size = 5)))+
  annotate("text", x =4.6, y = 1.5, fontface="bold",label = "c P conc.", size=8,colour="black")+labs(x="")
...
Fig_res_amb_Nc<-Fig_res_amb_Nc+geom_point(aes(group=Case_study,col=Country,size=Se,shape=Fraction),stroke=1.3,alpha=0.5)+scale_color_manual(values=c("#ffa600","black","#665191","#af060f"))##1C39A8 "#1dabe6","#b35a2d","#c3ced0","#ffa600","#665191","#af060f"        #"#ffa600","black","#665191","#af060f"
...
Fig_res_amb_Nc<-Fig_res_amb_Nc+theme_bw()+scale_shape_discrete(solid=F)+geom_segment(aes(x=1, y=0, xend=36, yend=0), lty=2, color = "magenta", cex=1.4)+
  ...
  theme(axis.title.x =element_text(size=24),
        ...
        legend.title = element_text(size=22),legend.box="vertical",legend.position="none")+ 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+guides(color = guide_legend(override.aes = list(size = 3)),shape = guide_legend(override.aes = list(size = 5)))+
  annotate("text", x =4.6, y = 1.5, fontface="bold",label = "d N conc.", size=8,colour="black")+labs(x="")
Fig_res_amb_Nc<-Fig_res_amb_Nc+
  guides(colour = guide_legend(nrow = 5, byrow = T, override.aes=list(size=3))) +
  guides(shape = guide_legend(nrow = 4, byrow = T, override.aes=list(size=4)),size=FALSE) +
  theme(legend.direction = "vertical", legend.box = "vertical")+labs(color="",shape="")
Fig_res_amb_Nc<-Fig_res_amb_Nc+xlab("Time since disturbance (Months)")
Fig_res_amb_Nc
...
#Saving in high res
ggsave(filename = "Fig_Resilience_NP_v3.png",
...
ggsave(filename = "Fig7_Resilience_NP_v9.png",
       plot = FigNP_v8, width = 23, height = 14, units = 'cm',
       scale = 2, dpi = 1000)
